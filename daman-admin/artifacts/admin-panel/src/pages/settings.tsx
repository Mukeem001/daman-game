...
import { useGetSettings, useUpdateSettings } from "@/lib/api-hooks";
...
export default function Settings() {
  ...
  const fetchUpiMethods = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/admin/settings/upi-methods`, {
        method: "GET",
        headers: {
  ...
  const fetchApkFiles = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${API_BASE_URL}/api/admin/settings/apk-files`, {
        method: "GET",
        headers: {
          "Authorization": token ? `Bearer ${token}` : "",
          "Content-Type": "application/json",
  ...
  const handleDeleteUpi = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${API_BASE_URL}/api/admin/settings/upi-methods/${id}`, {
        method: "DELETE",
        headers: {
          "Authorization": token ? `Bearer ${token}` : "",
          "Content-Type": "application/json",
  ...
  const handleDeleteApk = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${API_BASE_URL}/api/admin/settings/apk-files/${id}`, {
        method: "DELETE",
        headers: {
          "Authorization": token ? `Bearer ${token}` : "",
          "Content-Type": "application/json",
  ...
  const handleQrCodeUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${API_BASE_URL}/api/admin/settings/upload-qr`, {
        method: "POST",
        headers: {
          "Authorization": token ? `Bearer ${token}` : "",
        },
